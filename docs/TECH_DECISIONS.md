...
**Recommendation (superseded, see decisions log):** ~~Pagefind~~ — Pagefind crawls real static HTML at build time, which assumes a statically-generated multi-page site. PacketNova ended up as a client-rendered SPA (one `index.html` shell, React Router in history mode) with no prerendering step, so there's no per-page HTML for Pagefind's crawler to find. **Going with Fuse.js instead** — see the decisions log for the actual call made at Milestone 7.
...
| 2026-07-30 | History-mode routing (react-router) with the `404.html` redirect trick, per this doc's own Option B. | SEO on individual tool/visualizer/blog pages matters for organic search traffic, which is the recommendation's stated reasoning. The custom domain serves from root, so no path-prefix/basename complications.                                                                                                                                                                                   |
| 2026-07-30 | Fuse.js for client-side search, not this doc's original Pagefind pick.                               | Pagefind needs real per-route static HTML to crawl; PacketNova has no prerendering step, only one client-rendered `index.html` shell. Fuse.js runs directly against the structured content data (e.g. `content/reference/tools.ts`) that already exists in the bundle, no crawler or build step needed. Revisit if the Blog milestone later adds real prose content and prerendering gets built. |
