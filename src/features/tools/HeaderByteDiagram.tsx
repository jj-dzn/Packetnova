...
// the fixed grid.
export function HeaderByteDiagram({ fields }: HeaderByteDiagramProps) {
  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="flex justify-between px-1 font-mono text-[10px] text-fg-subtle">
        <span>bit 0</span>
        <span>bit 31</span>
      </div>
        ...
        <div key={rowIndex} className="flex h-11 overflow-hidden rounded-sm border border-border">
          {row.map(({ field, bits }, fieldIndex) => (
            <div
              ...
              className={`flex min-w-0 items-center justify-center overflow-hidden border-r border-border px-1 text-center font-mono text-[11px] last:border-r-0 ${TINTS[fieldIndex % TINTS.length]}`}
              title={`${field.field} -- ${field.size}${field.description ? ` -- ${field.description}` : ''}`}
            >
